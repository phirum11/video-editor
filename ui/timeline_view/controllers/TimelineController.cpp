...
struct AutoRippleGuard {
  TimelineController *c;
  bool wasRippling;
  AutoRippleGuard(TimelineController *c) : c(c) {
    wasRippling = c->m_isAutoRippling;
    c->m_isAutoRippling = true;
    if (!wasRippling) {
      c->undoStack()->beginMacro("Timeline Edit");
    }
  }
  ~AutoRippleGuard() {
    if (!wasRippling) {
...
TimelineController::TimelineController(QObject *parent)
    : QObject(parent), m_clipModel(new TimelineClipModel(this)),
  ...
  connect(&ActionManager::instance(), &ActionManager::actionTriggered, this,
  ...
  connect(m_vocalIsolator, &VocalIsolator::isolationFinished, this,
          [this](int clipIndex, int isolationType,
                 const QString &resultFilePath, bool success) {
            if (!success) {
            ...
            QString origPath = clip.originalFilePath.isEmpty()
                                   ? clip.filePath
                                   : clip.originalFilePath;
            m_clipModel->updateClipAudioSource(clipIndex, resultFilePath,
                                               origPath, isolationType);
            m_clipModel->updateClipIsolationProgress(clipIndex, -1);
  ...
  connect(m_vocalIsolator, &VocalIsolator::progressChanged, this,
          [this](int clipIndex, int progress) {
  ...
  connect(m_aiVoiceGenerator, &AIVoiceGenerator::progressChanged, this,
          [this](int clipIndex, int progress) {
            m_activeAIVoiceProgress = progress;
            emit activeAIVoiceProgressChanged();
          });

  connect(m_aiVoiceGenerator, &AIVoiceGenerator::segmentGenerated, this,
          [this](int clipIndex, const QString &jsonStr) {
            ...
            if (doc.isObject()) {
              ...
              // Generate a placeholder name based on the UI. We don't have an
              // exact index, but we can just call it "Generated Voice".
              addMediaAsset("Generated Voice", path, duration, false, true,
                            startSeconds, 3);
            }
          });

  connect(m_aiVoiceGenerator, &AIVoiceGenerator::generationFinished, this,
          [this](int clipIndex, const QString &metadataJsonPath, bool success) {
...
int TimelineController::playheadPosition() const { return m_playheadPosition; }
...
int TimelineController::addClip(const QString &clipName,
  ...
  if (hasVideo && isTrackLocked(true, targetTrack))
    return -1;
  if (!hasVideo && isTrackLocked(false, targetTrack >= 100 ? targetTrack - 100
                                                           : targetTrack))
    return -1;
...
int TimelineController::addMediaAsset(const QString &clipName,
  ...
  const double safeDuration =
      std::isfinite(durationSeconds) && durationSeconds > 0.0 ? durationSeconds
                                                              : 5.0;
  ...
  if (hasVideo && isTrackLocked(true, vTrack))
    return -1;
  if (hasAudio && !hasVideo && isTrackLocked(false, aTrack - 100))
    return -1;
  // Note: If it has both, we'd check both, but we'll let it slide if one is
  // locked or just prevent.
  if (hasVideo && hasAudio &&
      (isTrackLocked(true, vTrack) || isTrackLocked(false, aTrack - 100)))
    return -1;
  ...
  if (videoRow >= 0)
    selected.append(videoRow);
  if (audioRow >= 0)
    selected.append(audioRow);
...
bool TimelineController::deleteClip(int row) {
  ...
  if (isTrackLocked(!isAudioTrack, logicalIndex))
    return false;

  m_undoStack->push(
      new RemoveClipCommand(m_clipModel, row, RemoveClipCommand::SingleRow));
...
bool TimelineController::deleteLinkedClip(int row) {
  ...
  for (int i = 0; i < m_clipModel->rowCount(); ++i) {
    if (m_clipModel->linkGroupAt(i) == linkGroup) {
      ...
      if (isTrackLocked(!cIsAudio, logicalIndex))
        return false;
    }
  }

  m_undoStack->push(new RemoveClipCommand(
      m_clipModel, row, RemoveClipCommand::LinkedGroup, linkGroup));
  setSelectedClipIndices(QVariantList());
  return true;
}
...
int TimelineController::removeClipsByFilePath(const QString &filePath) {
  ...
  m_undoStack->push(new RemoveClipCommand(
      m_clipModel, -1, RemoveClipCommand::FilePath, filePath));
...
bool TimelineController::moveClip(int row, double startSeconds, int trackIndex,
  ...
  if (isTrackLocked(!isAudioTrack, srcLogicalIndex))
    return false;
...
bool TimelineController::moveSelectedClips(double deltaSeconds,
  ...
  for (const QVariant &v : m_selectedClipIndices) {
    int idx = v.toInt();
    if (idx < 0 || idx >= m_clipModel->rowCount())
      continue;

    if (linked) {
      QString linkGroup = m_clipModel->linkGroupAt(idx);
      if (!linkGroup.isEmpty()) {
        if (processedLinkGroups.contains(linkGroup))
          continue;
...
int TimelineController::splitClip(int row, double splitSeconds, bool linked) {
  ...
  int srcLogicalIndex =
      isAudioTrack ? srcClip.trackIndex - 100 : srcClip.trackIndex;
  if (isTrackLocked(!isAudioTrack, srcLogicalIndex))
    return -1;

  m_undoStack->beginMacro("Split Clip");

  const QString rightLinkGroup =
      QStringLiteral("clip_%1").arg(m_nextLinkGroupId++);
  ...
  if (linked && !linkGroup.isEmpty()) {
    for (int i = m_clipModel->rowCount() - 1; i >= 0; --i) {
      ...
      if (isTrackLocked(!cIsAudio, cLogical))
        continue;
      ...
      if (splitSeconds > start + 0.05 && splitSeconds < end - 0.05) {
        m_undoStack->push(
            new SplitClipCommand(m_clipModel, i, splitSeconds, rightLinkGroup));
    ...
    if (splitSeconds > start + 0.05 && splitSeconds < end - 0.05) {
      m_undoStack->push(
          new SplitClipCommand(m_clipModel, row, splitSeconds, rightLinkGroup));
      newSelectedRow = m_clipModel->rowCount() - 1;
    }
  }
...
void TimelineController::clearClips() {
  m_undoStack->push(
      new RemoveClipCommand(m_clipModel, -1, RemoveClipCommand::All));
  setSelectedClipIndices(QVariantList());
}
...
void TimelineController::generateAIVoiceFromSrt(const QString &language) {
  ...
  if (!m_selectedClipIndices.isEmpty()) {
    ...
    if (firstSelected >= 0 && firstSelected < m_clipModel->rowCount()) {
      QString path =
          m_clipModel->clipMapAt(firstSelected)["filePath"].toString();
...
void TimelineController::setClipMuted(int clipIndex, bool muted) {
  if (clipIndex < 0 || clipIndex >= m_clipModel->rowCount())
    return;
...
void TimelineController::setVocalIsolation(int clipIndex, int isolationType) {
  ...
  if (isolationType == 0) {
    ...
    if (!clip.originalFilePath.isEmpty()) {
      m_clipModel->updateClipAudioSource(clipIndex, clip.originalFilePath,
                                         QString(), 0);
    }
    return;
  }
  ...
  if (!clip.hasAudio) {
    qWarning() << "Cannot isolate vocals from a clip without audio:"
               << clipIndex;
    m_clipModel->updateClipIsolationProgress(clipIndex, -1);
    return;
  }
  ...
  QString sourceFile =
      clip.originalFilePath.isEmpty() ? clip.filePath : clip.originalFilePath;
...
void TimelineController::pushTrimCommand(int row, double newStart,
                                         double newDuration, double newInPoint,
                                         bool linked) {
  if (row < 0 || row >= m_clipModel->rowCount())
    return;
  TimelineClip clip = m_clipModel->clipAt(row);
  ...
  m_undoStack->push(new TrimClipCommand(m_clipModel, row, newStart, newDuration,
                                        newInPoint, linked));
}
...
double TimelineController::clipStartSeconds(int row) const {
  return m_clipModel->clipMapAt(row)
      .value(QStringLiteral("startSeconds"), -1.0)
      .toDouble();
}

double TimelineController::clipDurationSeconds(int row) const {
  return m_clipModel->clipMapAt(row)
      .value(QStringLiteral("durationSeconds"), 0.0)
      .toDouble();
}
...
double TimelineController::clipEndSeconds(int row) const {
  ...
  if (!std::isfinite(start) || start < 0.0 || !std::isfinite(duration) ||
      duration <= 0.0) {
    return -1.0;
  }
...
void TimelineController::groupSelectedClips() {
  if (m_selectedClipIndices.size() < 2)
    return;
...
bool TimelineController::isTrackEmpty(bool isVideo, int trackIndex) const {
  ...
  for (int i = 0; i < m_clipModel->rowCount(); ++i) {
    TimelineClip clip = m_clipModel->clipAt(i);
    if (clip.hasVideo == isVideo &&
        clip.trackIndex == (isVideo ? trackIndex : trackIndex + 100)) {
      return false;
    }
  }
  return true;
...
void TimelineController::cleanupEmptyTracks() { calculateTrackCounts(); }

void TimelineController::setTrackLocked(bool isVideo, int trackIndex,
                                        bool locked) {
  if (isVideo) {
    if (m_videoTrackLocked.value(trackIndex, false) == locked)
      return;
    m_videoTrackLocked[trackIndex] = locked;
  } else {
    if (m_audioTrackLocked.value(trackIndex, false) == locked)
      return;
...
bool TimelineController::isTrackLocked(bool isVideo, int trackIndex) const {
  return isVideo ? m_videoTrackLocked.value(trackIndex, false)
                 : m_audioTrackLocked.value(trackIndex, false);
}

void TimelineController::setTrackHidden(bool isVideo, int trackIndex,
                                        bool hidden) {
  if (isVideo) {
    if (m_videoTrackHidden.value(trackIndex, false) == hidden)
      return;
    m_videoTrackHidden[trackIndex] = hidden;
  } else {
    if (m_audioTrackHidden.value(trackIndex, false) == hidden)
      return;
...
bool TimelineController::isTrackHidden(bool isVideo, int trackIndex) const {
  return isVideo ? m_videoTrackHidden.value(trackIndex, false)
                 : m_audioTrackHidden.value(trackIndex, false);
}

void TimelineController::setTrackMuted(bool isVideo, int trackIndex,
                                       bool muted) {
  if (isVideo) {
    if (m_videoTrackMuted.value(trackIndex, false) == muted)
      return;
    m_videoTrackMuted[trackIndex] = muted;
  } else {
    if (m_audioTrackMuted.value(trackIndex, false) == muted)
      return;
...
bool TimelineController::isTrackMuted(bool isVideo, int trackIndex) const {
  return isVideo ? m_videoTrackMuted.value(trackIndex, false)
                 : m_audioTrackMuted.value(trackIndex, false);
}

int TimelineController::addEffectClip(const QString &name,
  ...
  clip.isEffect = true;
  clip.sourceInPoint = 0.0;
  clip.sourceDuration = durationSeconds;

  m_undoStack->push(new AddClipCommand(m_clipModel, clip));

  const int row = m_clipModel->rowCount() - 1;
  m_clipModel->updateClipEffects(row, effects);
  setSelectedClipIndices({row});
  return row;
}

void TimelineController::autoRippleTrack(int trackIndex) {
  if (trackIndex != 0)
    return;

  QList<int> clipIndices;
  for (int i = 0; i < m_clipModel->rowCount(); ++i) {
    if (m_clipModel->clipAt(i).trackIndex == trackIndex) {
      clipIndices.append(i);
    }
  }

  std::sort(clipIndices.begin(), clipIndices.end(), [this](int a, int b) {
    return m_clipModel->clipAt(a).startSeconds <
           m_clipModel->clipAt(b).startSeconds;
  });

  double currentStart = 0.0;
  for (int idx : clipIndices) {
    TimelineClip clip = m_clipModel->clipAt(idx);
    if (std::abs(clip.startSeconds - currentStart) > 0.001) {
      m_undoStack->push(new MoveClipCommand(m_clipModel, idx, currentStart,
                                            trackIndex, false));

      QString linkGroup = clip.linkGroupId;
      if (!linkGroup.isEmpty()) {
        double delta = currentStart - clip.startSeconds;
        for (int j = 0; j < m_clipModel->rowCount(); ++j) {
          if (j == idx)
            continue;
          if (m_clipModel->linkGroupAt(j) == linkGroup) {
            TimelineClip linkedClip = m_clipModel->clipAt(j);
            m_undoStack->push(new MoveClipCommand(
                m_clipModel, j, linkedClip.startSeconds + delta,
                linkedClip.trackIndex, false));
          }
        }
      }
    }
