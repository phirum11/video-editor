...
Rectangle {
    ...
    readonly property real minPixelsPerSecond: 0.5
    readonly property real maxPixelsPerSecond: 200
    readonly property real pixelsPerSecond: pixelsForZoomValue(zoomValue)
    readonly property real contentWidth: Math.max(Math.max(1, timelineViewport.width), displayDuration * pixelsPerSecond)
    ...
    function chooseDisplayDuration(spanSeconds) {
        const viewportW = Math.max(1, timelineViewport.width)
        const safeSpan = Math.max(0, spanSeconds)
        
        if (safeSpan <= 0) {
            // CapCut-style empty timeline: show ~15 pixels per second 
            return viewportW / 15.0
        }
      
        // Add 15% or at least 30 seconds of padding after the final clip
        return safeSpan + Math.max(30, safeSpan * 0.15)
    }
    ...
    function pixelsForZoomValue(value) {
        ...
        const ratio = maxPixelsPerSecond / minPixelsPerSecond
        return minPixelsPerSecond * Math.pow(ratio, clampedZoom)
    }
    ...
    function syncPlaybackPosition() {
        ...
        const safePosition = Number.isFinite(playbackPosition)
            ? Math.max(0, Math.min(playbackPosition, timelineEndSeconds))
            : 0
                ...
                Item {
                    ...
                    TimelineCanvas {
                        ...
                        onEffectDropped: function(name, filePath, startSeconds, trackIndex) {
                            tracksRoot.effectDropped(name, filePath, startSeconds, trackIndex)
                            timelineViewport.forceActiveFocus()
                        }
                    }
