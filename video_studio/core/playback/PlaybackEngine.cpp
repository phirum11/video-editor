...
#include <QPainter>
#include <QDebug>
#include "core/effects/EffectProcessor.h"
                              ...
                              bool expectedVideo)
{
    ...
    m_clipName = name;
    m_filePath = path;
    m_duration = std::max(0.0, knownDuration);
    m_position = 0.0;
    ...
    if (expectedVideo) {
        openedAnything = openVideoDecoder(path);
    }
    ...
    if (m_duration <= 0.0) {
        if (m_videoFormat) {
            m_duration = contextDurationSeconds(m_videoFormat);
        } else if (m_audioFormat) {
            m_duration = contextDurationSeconds(m_audioFormat);
        }
        emit durationChanged();
    }
...
void PlaybackEngine::tick()
{
    ...
    if (m_duration > 0.0 && targetPosition >= m_duration) {
        seek(m_duration);
        pause();
        return;
    }
    ...
    if (m_hasVideo) {
        ...
        if (m_lastVideoFrameSeconds >= 0.0
            && targetPosition - m_lastVideoFrameSeconds > 1.0) {
            seekVideo(targetPosition, true);
        } else {
            const double lagSeconds = m_lastVideoFrameSeconds >= 0.0
                ? targetPosition - m_lastVideoFrameSeconds
                : 0.0;
            ...
            if (lagSeconds > resyncThreshold) {
                ...
                while (decodedFrames < 4
                   && (m_lastVideoFrameSeconds < 0.0
                       || m_lastVideoFrameSeconds + frameInterval * 0.55 < targetPosition)) {
                    QImage image;
                    ...
                    latestFrameSeconds = frameSeconds >= 0.0 ? frameSeconds : targetPosition;
